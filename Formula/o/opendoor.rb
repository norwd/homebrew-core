class Opendoor < Formula
  ...
  def install
    inreplace_files = ["opendoor.conf"]
    %w[base openvpn wireguard].each { |file| inreplace_files << "src/core/network/adapters/#{file}.py" }
    inreplace inreplace_files, "/opt/homebrew", HOMEBREW_PREFIX
